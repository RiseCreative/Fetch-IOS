...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection {
    ...
    float rate1, rate2;
    rate1 = image.size.width / image.size.height;
    rate2 = frame.size.width / frame.size.height;
    
    if (rate1 != rate2) {
        if (image.size.width > image.size.height) {
            frame.size.height = frame.size.width * 1.0f / image.size.width *image.size.height;
            
        } else {
            frame.size.width = frame.size.height * 1.0f / image.size.height *image.size.width;
        }
        self.frame = frame;
    }*/
    ...
    if ([self superview] != nil) {
        [_delegate imageLoaded:image url:urlString];
    }
    //self.contentMode = UIViewContentModeScaleAspectFit;
